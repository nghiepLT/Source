<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TTDongbonhanvien.ascx.cs"
     Inherits="WebCus.TTDongbonhanvien" %>


<div class="page-title">
    <h2 class="icon-title">
        <span>Đồng bộ nhân viên</span>
    </h2>
</div>
<div class="TboardBox">
    <hr />
                ...
                <asp:GridView ID="gvBanner" runat="server" Width="100%" AutoGenerateColumns="False"
                    ...
                    <Columns>
                       ...
                       <asp:TemplateField HeaderText="Đồng bộ dữ liệu">
                            <ItemTemplate>
                                  <asp:ImageButton Width="20" ID="imgDongBo" ImageUrl="/images/icon/sync-flat.png" ToolTip="Xóa"
                                    runat="server" CommandArgument='<%# Eval("Id") %>' CommandName="DongBoItem"
                                    OnClientClick="return confirm('Bạn thật sự muốn đồng bộ nhân viên này?')" />
                            </ItemTemplate>
                             <HeaderStyle CssClass="RB_C" />
                            <ItemStyle CssClass="RB_L" />
                        </asp:TemplateField>
...
<script>
    ...
    function ShowPopupMapLink4(id) {
        WindowOpen('RenderPopupThumoinhanviec2.aspx?id=' + id, 'POpi', 800, 550, 'no');
        return false;
    }
</script>
