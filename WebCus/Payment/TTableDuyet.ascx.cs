...
namespace WebCus
{
    public partial class TTableDuyet : CommonUserControl
    {
        ...
        private void BindGirdView()
        {
           ...
           // int songaynghi = 0;
            ...
            DataSet ds = blc_user.Casework_Rows(this.CurrentPage, int.MaxValue, fromDate, toDate, trangthai, loaiphep, loaisearch, txtSearch.Text.Trim(), iduserchoose, idparentPhongban, idnhomduyet, idtrPhong, idPhoPhong, idtrNhom, idcty);
        ...
        private void Update_Status(string status_trans)
        {
            ...
            try
            {
                ...
                if (status_trans == "1")//duyet
                {
                    ...
                    {
                       // Alert.Show("Success !");
                        lbl_satus.Text = "Tác vụ thành công !!!";
                        BindGirdView();
                    }
        ...
        protected void dropSearchtype_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (dropSearchtype.SelectedValue == "1")
            {
                ...
                txtSearch.Visible = false;
                dr_cty.Visible = true;
                if (this.UserMemberLike != -1 && this.UserMemberLike != 0)
                {
                    dr_cty.SelectedValue = this.UserMemberLike.ToString();
                    dr_cty.Enabled = false;
                    // dr_cty_SelectedIndexChanged();
                    IList<PhongBan> dt = blc_user.ListPhongban_byCTY(Utils.TryParseInt(this.UserMemberLike.ToString(), 0));
