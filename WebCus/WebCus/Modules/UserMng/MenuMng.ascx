...
<script type="text/javascript">
    ...
    function change_url() { 
        var url = $('#MainContent_ctl00_ctl00_dl_map_item').val();
        if(url != "#")
            $('#MainContent_ctl00_ctl00_txtUrl').val(url);

        var item_text = $("#MainContent_ctl00_ctl00_dl_map_item option:selected").text();
        if(item_text.indexOf('>') > 0)
            item_text = item_text.substring(item_text.indexOf('>') + 2, 100);

        $('#MainContent_ctl00_ctl00_txtMenuName').val(item_text);
        $('#MainContent_ctl00_ctl00_txtMenuName2').val(item_text);
        $('#MainContent_ctl00_ctl00_txtMenuName3').val(item_text);

	url = $('#ctl00_MainContent_ctl00_ctl00_dl_map_item').val();
        if(url != "#")
            $('#ctl00_MainContent_ctl00_ctl00_txtUrl').val(url);

        item_text = $("#ctl00_MainContent_ctl00_ctl00_dl_map_item option:selected").text();
        if(item_text.indexOf('>') > 0)
            item_text = item_text.substring(item_text.indexOf('>') + 2, 100);

        $('#ctl00_MainContent_ctl00_ctl00_txtMenuName').val(item_text);
        $('#MainContent_ctl00_ctl00_txtMenuName2').val(item_text);
        $('#MainContent_ctl00_ctl00_txtMenuName3').val(item_text);
    }
                    ...
                    <tr>
                        ...
                        <td class="B_L">
                           <%-- <asp:TextBox ID="txtParentID" runat="server" CssClass="Input_text" Width="60px"></asp:TextBox>--%>
                             <asp:DropDownList ID="ddl_parent" Width="170" runat="server">
                            </asp:DropDownList>
                        </td>
