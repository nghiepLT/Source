...
namespace WebCus
{
    public partial class TTThumoinhanviec : System.Web.UI.UserControl
    {
        ...
        private void BindGird()
        {
            ...
            IList<UngVien> list = blc_user.GetListUngVien().Where(m=>m.IdNhanVien==null).Where(m => m.Status == 2).ToList();
        ...
        public string checkchucnang(string status, string id)
        {
            ...
            if (status == "2")
            {
                return "<a onclick=\"ShowPopupMapLink3('" + Guid.Parse(id) + "')\" style='color:blue'>Mời nhận việc</a>";
            }
            return "";
        }
    }
